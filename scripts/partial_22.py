...
news_url_xpath_string = '//h3[@class="o-post-preview_title"]/a/@href'
...
def get_news_datetime(response):
    ...
    if xpath_element:
        ...
        date_datetime = datetime.strptime(date_string, '%Y-%m-%dT%H:%M:%S')
    else:
...
def get_news_content_html(response):
    ...
    xpath_elements = tree.xpath(
        '//div[@class="o-full-width-text-group" or @class="post_body"]/*'
    )
    if xpath_elements:
        content_list = []
...
def get_news_tag(response):
    ...
    xpath_elements_list = tree.xpath(
        '//a[@class="tags_link"]/text()[normalize-space()]'
    )
